...
In this exercise we will practice creating some simple network visualisations using a fictional network of East Asian gangsters and revolutionaries.

This is an R Notebook which can be . You can run all of the code here in one go with cmd/ctrl-option-r or you can run code from  a single section using cmd/ctrl-shift-enter

For this exercise you need the packages: readr igraph dplyr tibble ggplot2 ggraph tidygraph

Choose **"Install Packages..."** from the **Tools** menu and enter the above packages in separated by a space. After the packages are already installed (you may have had some of them already), then load them.
...
Now we need to create a network object from our nodes and edges:
...
Up until now we have been using the Kamada-Kawai layout algorithm to determine the look of our network. There are a range of the other layouts you can create with the replacement of the layout type.
...
- [Jesse Sadler's Tutorial on ggraph](https://www.jessesadler.com/post/network-analysis-with-r/)
...
- [Another Game of Thrones network analysis – this time with tidygraph and ggraph](https://www.r-bloggers.com/another-game-of-thrones-network-analysis-this-time-with-tidygraph-and-ggraph/)
...
*This was written with the help of various books mentioned above, but mostly thanks to 40-60 google searches, with the answers found generally on the websites above, Stack Overflow, and obscure online bulletin boards.*
